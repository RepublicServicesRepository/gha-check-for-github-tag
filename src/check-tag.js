...
async function getMatchingTags (request) {
    console.log('Searching for tag: %s', request.tag)
    ...
    try {
        ...
        if (error.status === 404) {
            core.error('Unable to find repository %s/%s', request.owner, request.repository)
        } else {
...
async function tagExists (request) {
    ...
    if (tagExists) {
        console.log('A matching tag was found: %s', JSON.stringify(response.data))
    } else {
